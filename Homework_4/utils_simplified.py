...
def gradientDescent(f_pars_0: np.ndarray, f: typing.Callable, f_gradient: typing.Callable,
    ...
    f_pars_estimate = f_pars_0
    grad_0 = f_gradient(f_pars_0)

    for i in range(kmax):
        if (i + 1) % 100 == 0 and verbose:
            print("Running iteration %d" % (i + 1))
        grad = f_gradient(f_pars_estimate)

        if use_backtracking:
            alpha = backtracking(f, f_gradient, f_pars_estimate)

        f_pars_estimate_i = f_pars_estimate.copy()
        f_pars_estimate = f_pars_estimate - alpha * grad
        if reachedConvergence(f_pars_estimate, f_pars_estimate_i, grad_0, grad, tolf, tolx):
...
def MLEPolynomialRegression(data: typing.Tuple[np.ndarray, np.ndarray], k: int, sigma_estimate: float,
    ...
    if solving_strategy == "gd":
        ...
        theta_estimates = gradientDescent(theta, loss, loss_gradient, kmax=kmax, verbose=False, use_backtracking=True)
